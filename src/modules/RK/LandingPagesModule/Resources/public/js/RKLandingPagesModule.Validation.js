...
function rKLandingPagesToday(format)
{
...
function rKLandingPagesReadDate(val, includeTime)
{
 ...
 */
function rKLandingPagesUniqueCheck(ucOt, val, elem, ucEx)
{
    var result, params;

    if (elem.val() == window['last' + rKLandingPagesCapitaliseFirstLetter(ucOt) + rKLandingPagesCapitaliseFirstLetter(elem.attr('id')) ]) {
        return true;
    }

    window['last' + rKLandingPagesCapitaliseFirstLetter(ucOt) + rKLandingPagesCapitaliseFirstLetter(elem.attr('id')) ] = elem.val();
    ...
    jQuery.ajax({
        url: Routing.generate('rklandingpagesmodule_ajax_checkforduplicate'),
        datatype: 'json',
        async: false,
        data: params,
...
function rKLandingPagesValidateNoSpace(val)
{
    var valStr;
...
function rKLandingPagesValidateUploadExtension(val, elem)
{
 ...
 */
function rKLandingPagesExecuteCustomValidationConstraints(objectType, currentEntityId)
{
    jQuery('.validate-nospace').each( function() {
        if (!rKLandingPagesValidateNoSpace(jQuery(this).val())) {
    ...
    jQuery('.validate-upload').each( function() {
        if (!rKLandingPagesValidateUploadExtension(jQuery(this).val(), jQuery(this))) {
    ...
    jQuery('.validate-unique').each( function() {
        if (!rKLandingPagesUniqueCheck(jQuery(this).attr('id'), jQuery(this).val(), jQuery(this), currentEntityId)) {
