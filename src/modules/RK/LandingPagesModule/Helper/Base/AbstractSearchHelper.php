...
abstract class AbstractSearchHelper implements SearchableInterface
{
    ...
    {
        if (!$this->permissionApi->hasPermission('RKLandingPagesModule::', '::', ACCESS_READ)) {
            return '';
        }
    ...
    public function getResults(array $words, $searchType = 'AND', $modVars = null)
    {
        ...
        $searchTypes = $this->getSearchTypes();
        $entitiesWithDisplayAction = ['page', 'image'];
        ...
        foreach ($searchTypes as $searchTypeCode => $typeInfo) {
            ...
            if (isset($moduleActivationInfo['RKLandingPagesModule'])) {
                $moduleActivationInfo = $moduleActivationInfo['RKLandingPagesModule'];
            ...
            foreach ($entities as $entity) {
                ...
                if (!$this->permissionApi->hasPermission('RKLandingPagesModule:' . ucfirst($objectType) . ':', $entity->getKey() . '::', ACCESS_OVERVIEW)) {
                    continue;
                }
                ...
                if ($hasDisplayAction) {
                    ...
                    $displayUrl = new RouteUrl('rklandingpagesmodule_' . strtolower($objectType) . '_display', $urlArgs);
                }
    
                $result = new SearchResultEntity();
                $result->setTitle($formattedTitle)
                    ->setText($description)
                    ->setModule('RKLandingPagesModule')
                    ->setCreated($created)
    ...
    {
        $searchTypes = [
            'rKLandingPagesModulePages' => [
                'value' => 'page',
                'label' => $this->__('Pages')
            ],
            'rKLandingPagesModuleImages' => [
                'value' => 'image',
