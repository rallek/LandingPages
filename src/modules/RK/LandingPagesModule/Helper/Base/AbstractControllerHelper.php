...
use RK\LandingPagesModule\Helper\CollectionFilterHelper;
use RK\LandingPagesModule\Helper\ImageHelper;
use RK\LandingPagesModule\Helper\ModelHelper;
...
abstract class AbstractControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in RKLandingPagesModule.
     *
    ...
    /**
     * Returns the default object type in RKLandingPagesModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        $templateParameters['own'] = $request->query->getInt('own', $this->variableApi->get('RKLandingPagesModule', 'showOnlyOwnEntries', 0));
        ...
        if ($templateParameters['all'] != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('RKLandingPagesModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('RK\LandingPagesModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('rklandingpagesmodule_' . strtolower($objectType) . '_view', $urlParameters);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('rklandingpagesmodule_' . strtolower($objectType) . '_display', $urlParameters);
        }
    ...
    public function addTemplateParameters($objectType = '', array $parameters = [], $context = '', array $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if ($objectType == 'image') {
                $thumbRuntimeOptions = [];
                $thumbRuntimeOptions[$objectType . 'Picture'] = $this->imageHelper->getRuntimeOptions($objectType, 'picture', $context, $args);
            ...
            if (in_array($args['action'], ['display', 'edit', 'view'])) {
                ...
                $parameters['relationThumbRuntimeOptions'] = $this->imageHelper->getCustomRuntimeOptions('', '', 'RKLandingPagesModule_relateditem', $context, $args);
            }
        }
