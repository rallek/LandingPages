...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
     ...
     * RKLandingPagesModule will use this to add a javascript helper to add custom items.
     *
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $targetDir = 'web/modules/rklandingpages';
        $finder = new Finder();
        if (!$this->filesystem->exists($targetDir)) {
            $this->filesystem->mkdir($targetDir, 0777);
            if (is_dir($originDir = 'modules/RK/LandingPagesModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/RK/LandingPagesModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                'module' => 'RKLandingPagesModule',
                'type' => 'javascript',
                'path' => $this->request->getBasePath() . '/web/modules/rklandingpages/js/RKLandingPagesModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rklandingpagesmodule',
            'path' => $this->request->getBasePath() . '/web/modules/rklandingpages/scribite/CKEditor/rklandingpagesmodule/',
            'file' => 'plugin.js',
            'img' => 'ed_rklandingpagesmodule.gif'
        ]);
    }
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rklandingpagesmodule',
            'path' => $this->request->getBasePath() . '/web/modules/rklandingpages/scribite/Quill/rklandingpagesmodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rklandingpagesmodule',
            'path' => $this->request->getBasePath() . '/web/modules/rklandingpages/scribite/Summernote/rklandingpagesmodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rklandingpagesmodule',
            'path' => $this->request->getBasePath() . '/web/modules/rklandingpages/scribite/TinyMce/rklandingpagesmodule/plugin.js'
        ]);
    }
}
